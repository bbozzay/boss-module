...
export const login = ({ $auth }, params) => {
  try {
    $auth.loginWith('auth0', params)
    // First time login
...
export const loginThenRedirect = ({ state, $auth, namespace }, { url, screen_hint }) => {
  const stateUrl = url ? encodeState(url) : encodeState(state[`${namespace}`].redirectUrl);
  const params = () => {
    return stateUrl ? {
      params: {
        screen_hint: screen_hint ? screen_hint : "login",
        state: stateUrl
      }
    } : null;
  }
